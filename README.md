...
- **🌟 语音迭代改图(核心差异化)**:出图后直接说「把猫改成橙色」「背景换成星空」「右边加只狗」,系统**以当前图为参考重绘——只改你说的、其余保持一致**(角色 / 构图连贯);说「撤销」回上一张。这是和"通用文生图每次重抽"的本质区别:你在用语音做**美术指导**。
...
5. **看到图后用语音改它**:「**把猫改成橙色**」「**背景换成星空**」「**右边加只小狗**」——系统以当前图为参考重绘,**只改你说的、其余保持一致**;说「**撤销**」回上一张。
6. 想要全新一张就说「**重新画一张……**」;说「**新对话**」或点右上「🆕 新对话」清空记忆从头开始。
