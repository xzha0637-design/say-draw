...
```
语音(ASR) → 和豆包多轮对话(带记忆,缺信息就反问) → 用户确认 → 豆包综合出完整画面描述 → Seedream 文生图 → 最终图片
                       ▲ 语音朗读(TTS)回应每一轮 ▼
```

"画"发生在对话里,真正的成品是生成的图;全程语音进、语音出,不碰键鼠。
...
- **最终图生成 + 继续改**:Seedream 出图后,可说「返回」继续聊,或直接说怎么改(换背景 / 换风格…)再生成。
...
4. 说「**可以了 / 生成吧**」→ AI 综合对话生成完整描述 → Seedream 出**最终图片**(覆盖层展示)。
5. 看完说「**返回**」或点击任意处回到对话;可继续说「**把背景换成星空**」改了再生成。
6. 说「**新对话**」或点右上「🆕 新对话」清空记忆、从头开始。
